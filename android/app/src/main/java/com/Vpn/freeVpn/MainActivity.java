...
public class MainActivity extends FlutterActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // Request notification permission on Android 13+ so foreground VPN notification shows on real devices
    ...
    private void requestNotificationPermissionIfNeeded(boolean force) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) return; // not needed
        if (ContextCompat.checkSelfPermission(this, android.Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED) {
            if (force) Toast.makeText(this, "Notifications already allowed", Toast.LENGTH_SHORT).show();
            return;
        }
        // Show system dialog
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQ_POST_NOTIFICATIONS) {
            ...
            if (!granted) {
                boolean showRationale = ActivityCompat.shouldShowRequestPermissionRationale(this, android.Manifest.permission.POST_NOTIFICATIONS);
                if (!showRationale) {
                    // Permanently denied
                    Toast.makeText(this, "Enable notifications in Settings for VPN status", Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(this, "Notification permission needed for VPN control", Toast.LENGTH_LONG).show();
                }
            } else {
    ...
    private void updateVPNStatus() {
        if (attached && vpnStatusSink != null) {
            if (localJson != null) {
                vpnStatusSink.success(localJson.toString());
            } else {
                try {
                    JSONObject placeholder = new JSONObject();
                    placeholder.put("duration", "00:00:00");
                    placeholder.put("last_packet_receive", "0");
                    placeholder.put("byte_in", "0");
                    placeholder.put("byte_out", "0");
                    vpnStatusSink.success(placeholder.toString());
